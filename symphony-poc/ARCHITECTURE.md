...
- The disposable devcontainer mounts the local `symphony-poc-gh-auth` volume
  at `/home/node/.config/gh`. GitHub CLI browser/device authentication and
  organization SSO state persist there; Codex authentication is not a GitHub
  credential, and issue workspaces receive no credential copy.
